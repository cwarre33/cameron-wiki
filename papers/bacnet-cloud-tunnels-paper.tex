...
% Title
\title{\textbf{External-FDT}: Cloud-to-Building Control System Tunneling\\
       in Internet-Exposed BACnet Networks}
...
phenomenon where a single external endpoint serves multiple distinct building 
networks. Our empirical analysis of 17 seeded BBMDs (longitudinally sampled over 
...
broadcast networks. The BACnet protocol (ISO 16484-5) enables these systems 
...
typically confined to private networks.
...
infrastructure including public cloud providers.

This finding is distinct from previously characterized ICS exposure patterns:
\begin{itemize}
  \item Not merely \emph{open ports}---functional tunneling relationships
  \item Not merely \emph{misconfiguration}---systemic cloud integration pattern
  \item Not merely \emph{single-building}---cross-building concentrator behavior
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Attribution Framework}: We validate methods for 
    attributing external endpoints to cloud providers through IP-to-ASN 
    and geolocation analysis without active probing.
\end{enumerate}
...
enable BACnet to span IP subnets by:
...
\begin{itemize}
  \item Protocol vulnerabilities (Modbus, DNP3, BACnet object enumeration)
  \item Shodan-based exposure quantification (port scans, device counts)
  \item Specific CVE exploitation
\end{itemize}
...
during normal internet scanning. We performed:
\begin{itemize}
